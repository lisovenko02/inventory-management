...
type AggregateData = {
  [category: string]: AggregateDataItem
}
...
const Expenses = () => {
  ...
  const aggregatedData: AggregateDataItem[] = useMemo(() => {
    const filtered: AggregateData = expenses
      ...
      .reduce((acc: AggregateData, data: ExpenseByCategorySummary) => {
        ...
        <div className="flex-grow bg-white shadow rounded-lg p-4 md:p-6">
          <ResponsiveContainer width="100%" height={400}>
            <PieChart>
              ...
              >
                {aggregatedData.map(
                  (entry: AggregateDataItem, index: number) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={
